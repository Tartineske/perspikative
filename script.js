...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!menuIcon) return; // sécurité si l'élément n'existe pas

  let menuOuvert = false; // false = menu.svg, true = fermer.svg

  menuIcon.addEventListener('click', (event) => {
    event.preventDefault();

    if (!menuOuvert) {
      menuIcon.src = 'logos/fermer.svg';
      menuOuvert = true;
    } else {
      menuIcon.src = 'logos/menu.svg';
      menuOuvert = false;
    }
  });
});
